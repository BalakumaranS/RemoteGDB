...
return {
init:function(){$("#pyvideo,#pydta,#pyhertz,#pytv").on('click',this.showSettings);$("#menu_pyvideo_form,#menu_pydta_form,#menu_pyhertz_form,#menu_pytv_form").on('submit',this.callPyBlock);$("#up,#down,#channel").on('click',eevaa.callPyRemote);$("#pause").on('click',function(){eevaa.pytvtimer?clearInterval(eevaa.pytvtimer):""});$("#trash").on('click',eevaa.trash)},
...
var $this=$(this),tid=$this.attr('id'),urli=tid.split("_")[1],url=(urli=="pytv")?"pyvideo":urli,urlmain="http://"+device+":"+port,urlpics=urlmain+"/images/captures/",urlaudio=urlmain+"/Recordings/",urltools="http://"+device+":"+port+"/tools/",urlpath=urltools+url,valid={};
...
c=ims+c;$("#testoutput").html(c);$("#trash").attr('data-file','').attr('data-file','images/captures/'+valid.testname+'1.jpeg images/captures/'+valid.testname+'2.jpeg');break;
case "pytv":
var ims='<div class="row"><div class="col-md-10 col-sm-10 col-lg-10"><img src="'+urlpics+valid.testname+'.jpeg" class="img-rounded" alt="Image before" /></div></div>';$("#testoutput").html(ims);$("#trash").attr('data-file','').attr('data-file','images/captures/'+valid.testname+'.jpeg');
eevaa.pytvtimer=setInterval(function(){
$.post(urlpath+".py",valid).done(function(){var ims='<div class="row"><div class="col-md-10 col-sm-10 col-lg-10"><img src="'+urlpics+valid.testname+'.jpeg" class="img-rounded" alt="Image" /></div></div>';$("#testoutput").html(ims);
}).fail(function(er){var m='<div class="alert alert-warning alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><strong>Error:</strong>'+er+'</div>';$("#testoutput").html(m);});
},1000);break;
case "pyhertz":
default:
