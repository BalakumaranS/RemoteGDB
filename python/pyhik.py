...
### Load the dll                                 ###
####################################################
_pythonMode = 8 * struct.calcsize('P')
HC = None
try:
	if _pythonMode == 32:
		HC = ctypes.WinDLL('.\\dlls\\HCNetSDK_32.dll')
	elif _pythonMode == 64:
		HC = ctypes.WinDLL('.\\dlls\\HCNetSDK_64.dll')
except Exception, e:
	print('DLL not available')
	raise Exception(e)
...
__capdir__ = "C:\\EEVAA\\public\\images\\captures\\"
...
def captureDTADiag(ip,port,username,password,channelNo,img1name):
    """
    Start the capture for 16 video channels of the device and log the video status for every capture
    ...
    username - Username for login
    password - Password for login
    channelNo - Channel number (port) in the Hikvision
    img1name - Image 1 name
    ...
    createDirectory(channelNo,channelNo+1)

    jpegParam = NET_DVR_JPEGPARA()
    jpegParam.wPicSize = 18
    ...
    f=open(_ldir,'a')
    _cdir = __capdir__
    _cfilepath = os.path.join(_cdir,img1name)

    captureJPEGPicture(userId,channelNo,jpegParam,_cfilepath,f)
...
def captureDTAVideo(ip,port,username,password,channelNo,img1name,img2name):
    """
    Start the capture for 16 video channels of the device and log the video status for every capture
    ...
    duration - Duration for the test run
    img1name - Image 1 name
    img2name - Image 2 name

    @Return
    Capture images file paths
    """
    logger()
    f = open(os.path.join(__logdir__,'SDK.log'),'a')
    ...
    f=open(_ldir,'a')
    _cdir = __capdir__
    _cfilepath1 = os.path.join(_cdir,img1name)
    captureJPEGPicture(userId,channelNo,jpegParam,_cfilepath1,f)
    time.sleep(3)
    _cfilepath2 = os.path.join(_cdir,img2name)
    captureJPEGPicture(userId,channelNo,jpegParam,_cfilepath2,f)
    logout(userId,f)
    release(f)

    return _cfilepath1,_cfilepath2

if __name__ == "__main__":
    print(captureDTADiag("10.78.203.159",8000,"admin","12345"))